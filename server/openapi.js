...
const spec = {
  ...
  paths: {
    ...
    '/auth/reset-password': { post: { tags: ['auth'], summary: '密码重置（手机号+短信验证码；改密+解锁+吊销全部会话+自动登录）', responses: { '200': { description: 'ok', content: { 'application/json': { schema: { $ref: '#/components/schemas/LoginResp' } } } } } } },
    '/auth/login-logs': { get: { tags: ['auth'], summary: '登录日志查询（staff）：成功/失败/锁定/重置 + 异常统计', security: [{ bearerAuth: [] }], parameters: [{ name: 'date', in: 'query', schema: { type: 'string', description: 'YYYY-MM-DD，默认今天' } }, { name: 'status', in: 'query', schema: { type: 'string', enum: ['success', 'fail', 'lock', 'reset'] } }], responses: { '200': { description: 'ok' } } } },
